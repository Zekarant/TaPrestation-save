...
class UrgentSaleController extends Controller
{
    ...
    public function contact(Request $request, UrgentSale $urgentSale)
    {
        ...
        // Bloquer l'auto-contact pour les prestataires
        $user = Auth::user();
        if ($user && $user->role === 'prestataire' && $user->prestataire && $user->prestataire->id === $urgentSale->prestataire_id) {
            return back()->with('error', 'Vous ne pouvez pas contacter vous-même pour votre propre produit.');
        }
