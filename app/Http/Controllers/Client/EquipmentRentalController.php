...
use App\Notifications\EquipmentIssueReportedNotification;
use App\Notifications\EquipmentExtensionRequestedNotification;
use App\Notifications\EquipmentReturnConfirmedNotification;
use App\Notifications\EquipmentReviewSubmittedNotification;
...
class EquipmentRentalController extends Controller
{
    ...
    public function reportIssue(Request $request, EquipmentRental $rental)
    {
        ...
        $validated = $request->validate([
            ...
            'issue_photos.*' => 'image|mimes:jpeg,png,jpg,webp|max:5120',
        ...
        // Envoyer notification urgente au prestataire
        try {
            if ($rental->prestataire && $rental->prestataire->user) {
                $rental->prestataire->user->notify(new EquipmentIssueReportedNotification($rental, 'client'));
            }
        } catch (\Exception $e) {
            Log::error('Erreur notification problème équipement #' . $rental->id . ': ' . $e->getMessage());
        }
    ...
    public function requestExtension(Request $request, EquipmentRental $rental)
    {
        ...
        try {
            if ($rental->prestataire && $rental->prestataire->user) {
                $rental->prestataire->user->notify(new EquipmentExtensionRequestedNotification($rental, $extensionDays));
            }
        } catch (\Exception $e) {
            Log::error('Erreur notification prolongation #' . $rental->id . ': ' . $e->getMessage());
        }
    ...
    public function confirmReturn(Request $request, EquipmentRental $rental)
    {
        ...
        $validated = $request->validate([
            ...
            'return_condition_photos.*' => 'image|mimes:jpeg,png,jpg,webp|max:5120',
        ...
        try {
            if ($rental->prestataire && $rental->prestataire->user) {
                $rental->prestataire->user->notify(new EquipmentReturnConfirmedNotification($rental));
            }
        } catch (\Exception $e) {
            Log::error('Erreur notification retour confirmé #' . $rental->id . ': ' . $e->getMessage());
        }
    ...
    public function submitReview(Request $request, EquipmentRental $rental)
    {
        ...
        try {
            if ($rental->prestataire && $rental->prestataire->user) {
                $rental->prestataire->user->notify(new EquipmentReviewSubmittedNotification($rental));
            }
        } catch (\Exception $e) {
            Log::error('Erreur notification avis équipement #' . $rental->id . ': ' . $e->getMessage());
        }
    ...
    public function cancel(Request $request, EquipmentRental $rental)
    {
        ...
        // Gérer le remboursement via escrow si paiement effectué
        if (in_array($rental->payment_status, ['deposit_paid', 'full_paid'])) {
            try {
                ...
                if ($escrow) {
                    ...
                    if ($refundAmount > 0) {
                        $escrowService->refundClient($escrow->id, $refundAmount, 'Annulation location par le client - ' . $validated['cancellation_reason']);
                        $rental->update(['payment_status' => 'refund_pending']);
                    }
                }
            } catch (\Exception $e) {
                Log::error('Erreur remboursement annulation location client #' . $rental->id . ': ' . $e->getMessage());
            }
        }
        ...
        try {
            if ($rental->prestataire && $rental->prestataire->user) {
                $rental->prestataire->user->notify(new EquipmentRentalCancelledNotification($rental, 'client'));
            }
        } catch (\Exception $e) {
