...
class UrgentSaleController extends Controller
{
    ...
    {
        // For AJAX requests, reason is optional
        ...
        $urgentSale->update([
            'status' => 'suspended',
            'suspended_at' => now(),
            'suspension_reason' => $request->reason ?? 'Suspendu par l\'administrateur'
        ]);

        // Return JSON for AJAX requests
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json(['success' => true, 'message' => 'Vente urgente suspendue avec succès.']);
        }
     ...
     */
    public function destroy(Request $request, UrgentSale $urgentSale)
    {
        $urgentSale->delete();

        // Return JSON for AJAX requests
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json(['success' => true, 'message' => 'Vente urgente supprimée avec succès.']);
        }
