...
use App\Notifications\EquipmentSuspendedNotification;
use App\Notifications\EquipmentReactivatedNotification;
use App\Notifications\EquipmentDeletedByAdminNotification;
...
class EquipmentController extends Controller
{
    ...
    public function suspend(Request $request, Equipment $equipment)
    {
        ...
        // Notifier le prestataire de la suspension
        if ($equipment->prestataire && $equipment->prestataire->user) {
            try {
                $equipment->prestataire->user->notify(new EquipmentSuspendedNotification($equipment, $validated['reason']));
            } catch (\Exception $e) {
                \Log::error('Failed to notify prestataire of suspension', ['error' => $e->getMessage()]);
            }
        }
    ...
    public function reactivate(Equipment $equipment)
    {
        ...
        // Notifier le prestataire de la réactivation
        if ($equipment->prestataire && $equipment->prestataire->user) {
            try {
                $equipment->prestataire->user->notify(new EquipmentReactivatedNotification($equipment));
            } catch (\Exception $e) {
                \Log::error('Failed to notify prestataire of reactivation', ['error' => $e->getMessage()]);
            }
        }
    ...
    public function destroy(Request $request, Equipment $equipment)
    {
        ...
        $activeRentals = $equipment->rentals()
                                  ->whereIn('status', ['confirmed', 'in_preparation', 'ready_for_delivery', 'delivered', 'in_use', 'ready_for_pickup'])
                                  ->count();
        ...
        // Log de suppression (seulement si la table existe)
        if (\Illuminate\Support\Facades\TableExistenceCache::has('equipment_deletions')) {
            ...
            \Log::info('Equipment deleted by admin', [
                'equipment_id' => $equipment->id,
                'equipment_name' => $equipment->name,
                'reason' => $validated['reason'],
                'deleted_by' => auth()->id(),
        ...
        $prestataireUser = ($equipment->prestataire && $equipment->prestataire->user) ? $equipment->prestataire->user : null;

        $equipment->delete();
        
        // Notifier le prestataire de la suppression
        if ($prestataireUser) {
            try {
                $prestataireUser->notify(new EquipmentDeletedByAdminNotification($equipmentName, $validated['reason']));
            } catch (\Exception $e) {
                \Log::error('Failed to notify prestataire of deletion', ['error' => $e->getMessage()]);
            }
        }
