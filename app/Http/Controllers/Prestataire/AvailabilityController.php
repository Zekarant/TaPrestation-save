    ...
    public function updateWeeklyAvailability(Request $request)
    {
        ...
        foreach ($daysData as $dayOfWeek => $data) {
            ...
            if ($availability) {
                ...
                // Gérer les temps de pause (optionnels)
                if (isset($data['break_start']) && !empty($data['break_start'])) {
                    $updateData['break_start'] = $data['break_start'];
                } else {
                    $updateData['break_start'] = null;
                }
                
                if (isset($data['break_end']) && !empty($data['break_end'])) {
                    $updateData['break_end'] = $data['break_end'];
                } else {
        ...
        // Si on vient d'un flux (ex: création de service), revenir automatiquement à la page d'origine
        $returnUrl = $this->normalizeInternalRedirectPath(session()->pull('availability_setup_return_url'));
        if ($returnUrl !== null) {
