...
use App\Notifications\EquipmentReviewResponseNotification;
use App\Notifications\EquipmentReviewReportedNotification;
use App\Models\User;
...
class EquipmentReviewController extends Controller
{
    ...
    public function submitResponse(Request $request, EquipmentReview $review)
    {
        ...
        if ($review->client && $review->client->user) {
            try {
                $review->load(['equipment', 'prestataire.user']);
                $review->client->user->notify(new EquipmentReviewResponseNotification($review));
            } catch (\Exception $e) {
                \Log::error('Failed to notify client of review response', ['error' => $e->getMessage()]);
            }
        }
    ...
    public function report(Request $request, EquipmentReview $review)
    {
        ...
        // Notifier les administrateurs du signalement
        try {
            $review->load('equipment');
            $admins = User::where('role', 'admin')->get();
            foreach ($admins as $admin) {
                $admin->notify(new EquipmentReviewReportedNotification($review, 'prestataire', $validated['report_reason']));
            }
        } catch (\Exception $e) {
            \Log::error('Failed to notify admins of review report', ['error' => $e->getMessage()]);
        }
