...
class Prestataire extends Model
{
    ...
    protected $fillable = [
        ...
        'rating_average',
        'total_reviews',
        // SÉCURITÉ H5: 'commission_prestataire_disabled' retiré de $fillable
        // Ce flag ne doit être modifiable que par l'admin (AdminSettingsController)
        // 'commission_prestataire_disabled',
        'total_projects',
        'is_featured',
        'featured_until',
        'subscription_type',
    ...
    protected $casts = [
        'is_verified' => 'boolean',
        'is_approved' => 'boolean',
        'commission_prestataire_disabled' => 'boolean',
        ...
        'food_pickup_enabled' => 'boolean',
        'food_delivery_radius_km' => 'integer',
        'food_delivery_base_fee' => 'decimal:2',
        'food_delivery_fee_per_km' => 'decimal:2',
        'food_min_order_delivery' => 'decimal:2',
        'food_min_order_pickup' => 'decimal:2',
        'food_free_delivery_above' => 'decimal:2',
        'food_estimated_prep_time' => 'integer',
        'food_delivery_schedule' => 'array'
    ];
    ...
    // $dates supprimé (déprécié Laravel 12, audit 3.4) - les dates sont dans $casts
    ...
    /**
     * Relation avec les réservations (directes ou via services)
     */
    ...
    /**
     * Récupère toutes les réservations du prestataire (y compris celles liées via services)
     * Utile pour les anciennes réservations qui n'ont pas de prestataire_id direct
     */
    public function allBookings()
    {
        return Booking::where(function ($q) {
            $q->where('prestataire_id', $this->id)
                ->orWhereHas('service', fn($s) => $s->where('prestataire_id', $this->id));
        });
    }
    ...
    /**
     * Relation avec les produits alimentaires (menu)
     */
    ...
    /**
     * Relation avec les commandes alimentaires
     */
    ...
    {
        return $this->hasOne(PrestataireVerificationRequest::class);
    }
    ...
    {
        return $this->cover_image ? storage_asset_url($this->cover_image) : null;
    }
    ...
    public function getCoverImageAttribute($value)
    {
        ...
        if (!$path) {
            $path = $this->attributes['photo'] ?? ($this->attributes['profile_image'] ?? null);
        }

        if (!$path && $this->relationLoaded('foodProducts')) {
            $path = $this->foodProducts
                ->pluck('image')
                ->filter()
                ->first();
        }
        ...
        return function_exists('normalize_storage_asset_path')
            ? normalize_storage_asset_path($path)
            : $path;
    }
    ...
    /**
     * Backwards-compatible accessor: return `profile_image` or fallback to `photo`.
     */
    public function getProfileImageAttribute($value)
    {
        $path = $value ?: ($this->attributes['photo'] ?? null);
    ...
    /**
     * Fournit une URL utilisable pour l'image de profil (compatibilité).
     */
    public function getProfileImageUrlAttribute()
    {
        $path = $this->profile_image ?: ($this->photo ?? null);
        return $path ? storage_asset_url($path) : null;
    }

    /**
     * Backwards-compatible alias for `photo` field.
     */
    public function getPhotoAttribute($value)
    {
        $path = $value ?: ($this->attributes['profile_image'] ?? ($this->attributes['photo'] ?? null));
    ...
    /**
     * Legacy accessor used by some views: `profile_photo`.
     */
    public function getProfilePhotoAttribute($value)
    {
        $path = $value ?: ($this->profile_image ?: ($this->photo ?? null));
    ...
    /**
     * Legacy URL accessor used by views: `profile_photo_url`.
     */
    public function getProfilePhotoUrlAttribute()
    {
        $path = $this->profile_photo ?: ($this->profile_image ?: ($this->photo ?? null));
        return $path ? storage_asset_url($path) : null;
    }

    /**
     * Provide `photo_url` alias.
     */
    public function getPhotoUrlAttribute()
    {
        $path = $this->photo ?: ($this->profile_image ?? null);
        return $path ? storage_asset_url($path) : null;
    }

    /**
     * Compatibilité: `rating` → `rating_average`
     */
    ...
    /**
     * Compatibilité: `reviews_count` → `total_reviews`
     */
    public function getReviewsCountAttribute()
    {
        return $this->total_reviews ?? $this->reviews()->count();
    }
