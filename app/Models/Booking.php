...
class Booking extends Model
{
    ...
    {
        // Audit 3.6: utiliser random_int + suffixe unique pour éviter les collisions
        do {
            $number = 'BK' . date('Ymd') . str_pad((string) random_int(1, 9999), 4, '0', STR_PAD_LEFT);
        } while (static::where('booking_number', $number)->lockForUpdate()->exists());
    ...
    /**
     * Scope: bookings belonging to a prestataire (direct or via service).
     */
    public function scopeForPrestataire($query, $prestataire)
    {
        $prestataireId = is_object($prestataire) ? $prestataire->id : $prestataire;
        return $query->where(function ($q) use ($prestataireId) {
            $q->where('prestataire_id', $prestataireId)
                ->orWhereHas('service', fn($s) => $s->where('prestataire_id', $prestataireId));
        });
    }
    ...
    private function updateAvailabilitySlots(bool $isBooked): void
    {
        ...
        /** @var \Illuminate\Database\Eloquent\Collection<int, \App\Models\Availability> $availabilities */
        ...
        foreach ($availabilities as $availability) {
            ...
            if (
                $this->timeSlot->start_datetime >= $availability->start_time &&
