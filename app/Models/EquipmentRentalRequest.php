...
class EquipmentRentalRequest extends Model
{
    ...
    /**
     * Scope: rental requests belonging to a prestataire (direct or via equipment).
     */
    public function scopeForPrestataire($query, $prestataire)
    {
        ...
        return $query->where(function ($q) use ($prestataireId) {
            $q->where('prestataire_id', $prestataireId)
                ->orWhereHas('equipment', fn($eq) => $eq->where('prestataire_id', $prestataireId));
        });
    }
    ...
    public function accept($response = null)
    {
        DB::transaction(function () use ($response) {
            // Lock l'équipement pour éviter la double réservation (audit 3.6)
            $equipment = \App\Models\Equipment::lockForUpdate()->find($this->equipment_id);

            if (!$equipment) {
                throw new \Exception('Équipement introuvable.');
            }

            if (!$equipment->isAvailableForPeriod($this->start_date, $this->end_date)) {
