...
class EquipmentRental extends Model
{
    ...
    // Alias (ancienne nomenclature)
    const STATUS_PREPARING = self::STATUS_IN_PREPARATION;
    const STATUS_ACTIVE = self::STATUS_IN_USE;
    // Statut dérivé (n'existe pas en DB, calculé via dates)
    ...
    /**
     * Utilisateur ayant récupéré/retourné le matériel.
     */
    ...
    /**
     * Utilisateur ayant annulé la location.
     */
    ...
    {
        return $query->whereIn('status', [self::STATUS_IN_USE])
    ...
    {
        DB::transaction(function () use ($condition, $damageReported, $damageDescription) {
            $updateData = [
            ...
            if ($this->isOverdue()) {
                ...
                $updateData['late_return_fee'] = $this->daily_rate * $daysLate * 0.5;
            }
    ...
    public function getFormattedStatusAttribute()
    {
        $statuses = [
            ...
            self::STATUS_IN_PREPARATION => 'En préparation',
            self::STATUS_READY_FOR_DELIVERY => 'Prêt pour livraison',
            self::STATUS_DELIVERED => 'Livrée',
            self::STATUS_IN_USE => 'En cours',
            self::STATUS_READY_FOR_PICKUP => 'Prêt pour retour',
    ...
    public function getStatusColorAttribute()
    {
        $colors = [
            ...
            self::STATUS_IN_PREPARATION => 'yellow',
            self::STATUS_READY_FOR_DELIVERY => 'indigo',
            self::STATUS_DELIVERED => 'blue',
