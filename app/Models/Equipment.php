...
class Equipment extends Model
{
    ...
    protected $casts = [
        ...
        'auto_accept_on_deposit' => 'boolean',
    ...
    /**
     * Scope pour les équipements disponibles sur une période donnée
     * Vérifie available_from/available_until et l'absence de locations actives chevauchantes
     */
    public function scopeAvailableForPeriod($query, string $startDate, string $endDate)
    {
        return $query
            ->where(function ($q) use ($startDate) {
                $q->whereNull('available_from')
                    ->orWhereDate('available_from', '<=', $startDate);
            })
            ->where(function ($q) use ($endDate) {
                $q->whereNull('available_until')
                    ->orWhereDate('available_until', '>=', $endDate);
            })
            ->whereDoesntHave('rentals', function ($rentalQ) use ($startDate, $endDate) {
                $rentalQ->active()
    ...
    public static function updateRentalStatuses()
    {
        ...
        /** @var \Illuminate\Database\Eloquent\Collection<int, EquipmentRental> $endedRentals */
        ...
        /** @var \Illuminate\Database\Eloquent\Collection<int, EquipmentRental> $startingRentals */
