...
<?php $__env->startSection('title', 'Location de matériel - TaPrestation'); ?>
...
<div class="tp-equipment-page pb-24 sm:pb-12">
        ...
        <div class="tp-hero-inner">
            ...
            <?php if($equipments->count() > 0): ?>
                <div class="tp-hero-stats">
                    <div class="tp-hero-stat"><i class="fas fa-check-circle"></i> <?php echo e($equipments->total()); ?> équipement<?php echo e($equipments->total() > 1 ? 's' : ''); ?></div>
                    <?php if(isset($categories) && $categories->count() > 0): ?>
                        <div class="tp-hero-stat"><i class="fas fa-th-large"></i> <?php echo e($categories->count()); ?> catégorie<?php echo e($categories->count() > 1 ? 's' : ''); ?></div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
    ...
    <div class="tp-filters-wrap">
        <?php echo $__env->make('components.filters.compact-filters', [
            'pageType' => 'equipment',
            'themeColor' => 'emerald',
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 mt-4 sm:mt-6">
        <?php if($equipments->count() > 0): ?>
            <div class="tp-results-count">
                <strong><?php echo e($equipments->total()); ?></strong> résultat<?php echo e($equipments->total() > 1 ? 's' : ''); ?><?php if(request('q')): ?> pour « <?php echo e(request('q')); ?> »<?php endif; ?>
            </div>
            ...
            <div class="tp-grid">
                <?php $__currentLoopData = $equipments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $equipment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <div class="tp-card" data-card-link role="link" tabindex="0" data-href="<?php echo e(route('equipment.show', $equipment)); ?>">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.media-image','data' => ['path' => $firstPhoto,'alt' => $equipment->name,'style' => 'width:100%;height:100%;object-fit:cover;display:block']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['path' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($firstPhoto),'alt' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($equipment->name),'style' => 'width:100%;height:100%;object-fit:cover;display:block']); ?>
...
<?php endif; ?>
                            ...
                            <?php endif; ?>
                            <div class="tp-card-time"><?php echo e($equipment->created_at->diffForHumans(null, true, true)); ?></div>
                            <?php if(isset($equipment->is_available)): ?>
                                <div class="tp-card-available <?php echo e($equipment->is_available ? 'is-available' : 'is-unavailable'); ?>">
                                    <?php echo e($equipment->is_available ? 'Dispo' : 'Indispo'); ?>
                        ...
                        <div class="tp-card-body">
                            <div class="tp-card-price"><?php echo e(number_format($equipment->price_per_day, 0, ',', ' ')); ?> € <small>/jour</small></div>
                            <div class="tp-card-title"><?php echo e($equipment->name); ?></div>
                            <?php if($equipment->brand || $equipment->model): ?>
                                <div class="tp-card-subtitle"><?php echo e(trim(($equipment->brand ?? '') . ' ' . ($equipment->model ?? ''))); ?></div>
                            <?php elseif($prestataire): ?>
                                <div class="tp-card-subtitle"><?php echo e($prestataire->company_name ?? $prestataire->first_name); ?></div>
                            <?php endif; ?>
                            <div class="tp-card-meta">
                                <?php if($city): ?><span><i class="fas fa-map-marker-alt"></i> <?php echo e(Str::limit($city, 18)); ?></span><?php endif; ?>
                                <?php if(is_numeric($ratingAvg) && (float)$ratingAvg > 0): ?><span><i class="fas fa-star"></i> <?php echo e(number_format((float)$ratingAvg, 1, '.', '')); ?><?php if(is_numeric($ratingCount) && (int)$ratingCount > 0): ?> (<?php echo e((int)$ratingCount); ?>)<?php endif; ?></span><?php endif; ?>
                                <?php if(isset($equipment->distance_km) && $equipment->distance_km !== null): ?><span><i class="fas fa-route"></i> <?php echo e($equipment->distance_km); ?> km</span><?php endif; ?>
                            </div>
 
                            <div class="tp-card-footer">
                                <?php if(auth()->guard()->check()): ?>
                                    <?php $isOwner = $prestataire && auth()->user()->id === ($prestataire->user_id ?? null); ?>
                                    <?php if(!$isOwner): ?>
                                        <a href="<?php echo e($canContact ? route('client.messaging.start', $prestataire) : route('equipment.show', $equipment)); ?>" class="tp-card-btn" onclick="event.stopPropagation()">Contacter</a>
                                        <a href="<?php echo e(route('equipment.show', $equipment)); ?>#reservation" class="tp-card-btn tp-card-btn-primary" onclick="event.stopPropagation()">Réserver</a>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <a href="<?php echo e(route('login')); ?>" class="tp-card-btn" onclick="event.stopPropagation()">Contacter</a>
                                    <a href="<?php echo e(route('login')); ?>" class="tp-card-btn tp-card-btn-primary" onclick="event.stopPropagation()">Réserver</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
            ...
            <?php if($equipments->hasPages()): ?>
                <div class="tp-pagination"><?php echo e($equipments->appends(request()->query())->links()); ?></div>
            <?php endif; ?>
        <?php else: ?>
            <div class="tp-empty">
                ...
                <a href="<?php echo e(route('equipment.index')); ?>" class="tp-empty-btn"><i class="fas fa-redo" style="font-size:10px"></i> Réinitialiser</a>
            </div>
        <?php endif; ?>
    </div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\wamp64\www\TaPrestation-master - Copie\resources\views/equipment/index-modern.blade.php ENDPATH**/ ?>