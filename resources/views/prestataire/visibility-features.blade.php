...
<div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto">
        <div class="mb-8">
            <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Fonctionnalités de Visibilité</h1>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    @foreach($prestataire->portfolio_images as $index => $image)
                                        <div class="relative group">
                                            <x-media-image :path="$image" alt="Portfolio {{ $index + 1 }}" class="w-full h-24 object-cover rounded-lg" />
                            ...
                            <div class="mt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    @foreach($prestataire->portfolio_videos as $index => $video)
                                        <div class="relative group">
                                            <video class="w-full h-32 object-cover rounded-lg" controls playsinline webkit-playsinline x-webkit-airplay="allow">
                                                @if( \Illuminate\Support\Facades\Storage::disk('public')->exists($video) )
                                                    @php
                                                    ...
                                                    @endphp
                                                    <source src="{{ \Illuminate\Support\Facades\Storage::url($video) }}" type="{{ $mimeType }}">
                                                @else
                                                    <!-- video missing or unsupported, no source provided -->
                                                @endif
                                            </video>
