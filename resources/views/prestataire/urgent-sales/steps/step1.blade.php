            ...
            <div class="bg-white rounded-xl shadow-lg border border-red-200 p-3 sm:p-4 md:p-6 mb-4 sm:mb-6">
                ...
                <form id="urgentSaleStep1Form" action="{{ route('prestataire.urgent-sales.create.step1.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 lg:gap-8">
                        ...
                        <!-- Options de paiement -->
                        <div class="md:col-span-2 mt-4 border-t border-red-200 pt-4">
                            @php($cashOnlyMode = function_exists('cash_only_mode') && cash_only_mode())
                            <h3 class="text-sm sm:text-base font-semibold text-red-800 mb-3 flex items-center gap-2">
                                <i class="fas fa-credit-card text-red-600"></i>Options de paiement
                            </h3>
                            ...
                            @if($cashOnlyMode)
                                ...
                                <div class="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-800">
                                    ...
                                    <p class="mt-1">Cette vente sera publiée en mode contact vendeur uniquement. Les acheteurs vous contacteront et paieront directement en main propre.</p>
                                </div>
                            @else
                                <div>
                                    <label for="payment_requirement" class="block text-xs sm:text-sm font-medium text-red-700 mb-1 sm:mb-2">
                                        Paiement requis pour acheter
                                    </label>
                                    <select id="payment_requirement" name="payment_requirement" 
                                            class="w-full px-2 sm:px-3 py-2 text-sm sm:text-base border border-red-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 @error('payment_requirement') border-red-500 @enderror">
                                        <option value="none" {{ old('payment_requirement', session('urgent_sale_creation.step1.payment_requirement', 'none')) == 'none' ? 'selected' : '' }}>
                                            Aucun (contact vendeur seulement)
                                        </option>
                                        <option value="full" {{ old('payment_requirement', session('urgent_sale_creation.step1.payment_requirement')) == 'full' ? 'selected' : '' }}>
                                            Paiement en ligne obligatoire
                                        </option>
                                    </select>
                                    <p class="text-xs text-gray-500 mt-1">
                                        <i class="fas fa-info-circle mr-1"></i>
                                        "Aucun" = l'acheteur vous contacte pour négocier.<br>
                                        "Paiement obligatoire" = l'acheteur paie directement via la plateforme.
                                    </p>
                                ...
                                <div id="earningsPreviewContainer" class="mt-4 {{ old('payment_requirement', session('urgent_sale_creation.step1.payment_requirement', 'none')) == 'full' ? '' : 'hidden' }}">
                                    <x-earnings-preview 
                                        type="urgent_sale" 
                                        inputId="price" 
                                        :initialPrice="old('price')" 
                                        color="red" 
                                    />
                                </div>
                            @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Gestion de l'affichage de l'aperçu des gains selon le mode de paiement
    const paymentRequirementSelect = document.getElementById('payment_requirement');
    const earningsPreviewContainer = document.getElementById('earningsPreviewContainer');
    
    if (paymentRequirementSelect && earningsPreviewContainer) {
        paymentRequirementSelect.addEventListener('change', function() {
            if (this.value === 'full') {
                earningsPreviewContainer.classList.remove('hidden');
                // Déclencher le recalcul du preview
                const priceInput = document.getElementById('price');
                if (priceInput) {
                    priceInput.dispatchEvent(new Event('input'));
                }
            } else {
                earningsPreviewContainer.classList.add('hidden');
            }
        });
    }
});
