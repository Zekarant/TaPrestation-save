                ...
                <form id="urgentSaleStep4Form" action="{{ route('prestataire.urgent-sales.store') }}" method="POST">
                    ...
                    <div class="space-y-4 sm:space-y-6">
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
                            <h4 class="text-xs sm:text-sm font-semibold text-blue-800 mb-2">
                                <i class="fas fa-credit-card mr-1 sm:mr-2"></i>Mode de paiement
                            </h4>
                            @php
                                $paymentReq = function_exists('normalize_payment_requirement_for_mode')
                                    ? normalize_payment_requirement_for_mode(session('urgent_sale_creation.step1.payment_requirement', 'none'))
                                    : session('urgent_sale_creation.step1.payment_requirement', 'none');
                                $cashOnlyMode = function_exists('cash_only_mode') && cash_only_mode();
                            @endphp
                            @if($cashOnlyMode || $paymentReq === 'none')
                                <p class="text-xs sm:text-sm text-blue-700">
                                    <i class="fas fa-handshake mr-1"></i>
                                    <strong>Contact vendeur uniquement</strong> - Les acheteurs vous contacteront pour organiser le paiement directement avec vous.
                                </p>
                            @else
                                <p class="text-xs sm:text-sm text-blue-700">
                                    <i class="fas fa-lock mr-1"></i>
                                    <strong>Paiement en ligne obligatoire</strong> - Les acheteurs paieront via la plateforme. L'argent sera sécurisé jusqu'à confirmation de réception.
                                </p>
                            @endif
                        </div>
                        
                        <!-- Consentement aux conditions de paiement (uniquement si paiement en ligne) -->
                        @if(!((function_exists('cash_only_mode') && cash_only_mode())) && session('urgent_sale_creation.step1.payment_requirement', 'none') === 'full')
                        @include('components.payment-consent', ['type' => 'urgent_sale', 'color' => 'red'])
                        @endif
