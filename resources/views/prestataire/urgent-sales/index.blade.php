...
@section('content')
<div class="min-h-screen bg-linear-to-br from-red-50 to-pink-100">
    <!-- Success message -->
    ...
    <div class="bg-white shadow-lg border-b-4 border-red-600">
        <div class="container mx-auto px-4 py-3 sm:py-4 md:py-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-red-900 mb-1">Ma Boutique</h1>
                    <p class="text-xs sm:text-sm text-red-700">Gérez vos annonces et ventes en ligne</p>
                </div>
                <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <a href="{{ route('prestataire.urgent-sales.select-inventory') }}" class="w-full sm:w-auto bg-gray-600 hover:bg-gray-700 text-white px-3 py-2 sm:px-4 sm:py-2.5 rounded-lg transition duration-200 shadow-md hover:shadow-lg text-center text-sm sm:text-base">
                        <i class="fas fa-boxes mr-1 sm:mr-2"></i>Depuis inventaire
                    </a>
                    <a href="{{ route('prestataire.urgent-sales.create') }}" class="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white px-3 py-2 sm:px-4 sm:py-2.5 md:px-6 md:py-3 rounded-lg transition duration-200 shadow-md hover:shadow-lg text-center text-sm sm:text-base">
                        <i class="fas fa-plus mr-1 sm:mr-2"></i>Nouvelle annonce
                    </a>
                </div>
            </div>
        </div>
                ...
                <div class="bg-white rounded-lg sm:rounded-xl shadow-lg border border-red-200 hover:shadow-xl transition duration-200 flex flex-col h-full">
                    ...
                    <div class="relative">
                        @if($sale->photos && count($sale->photos ?? []) > 0)
                            <x-media-image :path="$sale->photos[0]" :alt="$sale->title" class="w-full h-32 sm:h-40 md:h-48 object-cover rounded-t-lg sm:rounded-t-xl" />
                    ...
                    <div class="p-2.5 sm:p-3 md:p-4 flex flex-col flex-grow">
                        ...
                        <!-- Badges stock et inventaire -->
                        <div class="flex flex-wrap gap-1.5 mb-2 sm:mb-3">
                            <!-- Badge stock -->
                            @if($sale->quantity !== null)
                                @if($sale->quantity > 5)
                                    <span class="inline-flex items-center text-[10px] sm:text-xs text-green-700 bg-green-100 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-lg border border-green-200">
                                        <i class="fas fa-boxes mr-1"></i>Stock: {{ $sale->quantity }}
                                    </span>
                                @elseif($sale->quantity > 0)
                                    <span class="inline-flex items-center text-[10px] sm:text-xs text-orange-700 bg-orange-100 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-lg border border-orange-200">
                                        <i class="fas fa-exclamation-triangle mr-1"></i>Stock: {{ $sale->quantity }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center text-[10px] sm:text-xs text-red-700 bg-red-100 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-lg border border-red-200">
                            ...
                            @if($sale->inventory_item_id)
                                <span class="inline-flex items-center text-[10px] sm:text-xs text-blue-700 bg-blue-100 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-lg border border-blue-200">
                                    <i class="fas fa-link mr-1"></i>Inventaire
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="flex justify-between text-[10px] sm:text-xs text-red-600 mb-3 sm:mb-4">
                            ...
                            <span class="font-semibold"><i class="fas fa-envelope mr-0.5 sm:mr-1 text-[8px] sm:text-xs"></i>{{ $sale->contacts_count ?? 0 }} contacts</span>
                            <span><i class="fas fa-map-marker-alt mr-0.5 sm:mr-1 text-[8px] sm:text-xs"></i>{{ $sale->city }}</span>
                        </div>
                        
                        <!-- Actions -->
                        <div class="mt-auto pt-2 sm:pt-3 border-t border-red-100">
                            <!-- Bouton contacts (si contacts > 0) -->
                            @if(($sale->contacts_count ?? 0) > 0)
                                <a href="{{ route('prestataire.urgent-sales.contacts', $sale) }}" class="w-full mb-2 block bg-green-600 hover:bg-green-700 text-white text-center py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg text-[10px] sm:text-xs transition duration-200 shadow-md hover:shadow-lg">
                                    <i class="fas fa-users mr-0.5 sm:mr-1"></i>Gérer {{ $sale->contacts_count }} contact(s)
                                </a>
                            @endif
        ...
        <div class="min-h-full w-full flex items-end sm:items-center justify-center p-3">
            <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl p-4 sm:p-6 w-full sm:max-w-md border border-red-200 transform transition-all duration-200 scale-95 opacity-0 modal-show">
                <div class="text-center">
                    ...
                    <p id="urgentSaleTitle" class="text-base font-semibold text-red-700 mt-2"></p>
