...
<div class="bg-blue-50">
    <div class="container mx-auto px-2 sm:px-4 py-4 sm:py-6">
        <!-- PWA Back Button -->
        <div class="mb-4">
            <x-pwa-back-button :href="route('prestataire.dashboard')" />
        </div>
        ...
        <x-help-box type="info" title="💡 Comment gérer vos demandes" :dismissible="true" storageKey="bookings-guide-tip" class="mb-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div>
                    <p class="font-semibold mb-1">Statuts :</p>
                    <ul class="space-y-0.5">
                        <li>🟡 <strong>En attente</strong> – Action requise de votre part</li>
                        <li>🟢 <strong>Acceptée</strong> – Rendez-vous confirmé</li>
                        <li>🔴 <strong>Refusée</strong> – Demande déclinée</li>
                    </ul>
                </div>
                <div>
                    ...
                    <ul class="space-y-0.5">
                        ...
                        <li>📞 Contactez le client en cas de question</li>
                        <li>⭐ Un bon service = bonnes évaluations</li>
                    </ul>
                </div>
            </div>
        </x-help-box>
        ...
        if(isset($serviceBookings) && $serviceBookings->count() > 0) {
            foreach($serviceBookings as $booking) {
                ...
                // Get client rating info
                $clientUser = ($booking->client && $booking->client->user) ? $booking->client->user : null;
                $clientRating = $clientUser ? $clientUser->client_rating : null;
                $clientReviewsCount = $clientUser ? $clientUser->client_reviews_count : 0;
                ...
                $allRequests->push((object)[
                    ...
                    'client_reviews_count' => $clientReviewsCount,
        ...
        if(isset($equipmentRentalRequests) && $equipmentRentalRequests->count() > 0) {
            foreach($equipmentRentalRequests as $request) {
                // Get client rating info
                $clientUser = ($request->client && $request->client->user) ? $request->client->user : null;
                $clientRating = $clientUser ? $clientUser->client_rating : null;
                $clientReviewsCount = $clientUser ? $clientUser->client_reviews_count : 0;
                                    ...
                                    <div class="space-y-1.5 text-xs mb-3">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <div>
                                                ...
                                                {{-- Afficher la note du client --}}
                                                @if(isset($item->client_rating) && $item->client_rating)
                                                    <div class="flex items-center space-x-1 mt-0.5">
                                                        @for($i = 1; $i <= 5; $i++)
                                                            <svg class="w-3 h-3 {{ $i <= round($item->client_rating) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                            </svg>
                                                        @endfor
                                                        <span class="text-xs text-gray-600">{{ number_format($item->client_rating, 1) }}</span>
                                                        <span class="text-xs text-gray-400">({{ $item->client_reviews_count ?? 0 }})</span>
                                                    </div>
                                                @elseif(isset($item->client_reviews_count) && $item->client_reviews_count == 0)
                                                    <span class="text-xs text-gray-400">Nouveau client</span>
                                                @endif
                                            </div>
                                        </div>
        ...
        @else
            <x-empty-state 
                ...
                message="C'est le moment de développer votre visibilité ! Les demandes arrivent lorsque les clients découvrent vos services. Enrichissez votre profil pour attirer plus de réservations."
                :primaryAction="['url' => route('prestataire.services.index'), 'label' => 'Gérer mes services']"
                :secondaryAction="['url' => route('prestataire.equipment.index'), 'label' => 'Mes équipements']"
                :tips="[
                    'Ajoutez des photos attractives à vos services',
                    'Activez la réservation en ligne pour plus de facilité',
                    'Partagez votre QR Code sur les réseaux sociaux',
