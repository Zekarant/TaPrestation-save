            ...
            <div class="max-w-6xl mx-auto">
                ...
                <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6 border border-gray-200">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-1 sm:mb-2">Gestion des
                                vidéos</h1>
                            <p class="text-gray-600 text-sm sm:text-base">Gérez vos vidéos et modifiez leurs informations
                            </p>
                        </div>
                        <div class="flex gap-2 flex-wrap">
                            <a href="{{ route('prestataire.videos.record') }}"
                                class="inline-flex items-center px-3 py-2 sm:px-4 sm:py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 font-medium text-sm sm:font-bold">
                                <i class="fas fa-record-vinyl w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2"></i>
                                Enregistrer
                            </a>
                ...
                @if(session('success'))
                    <div
                        class="bg-green-100 border border-green-400 text-green-700 px-3 py-2 sm:px-4 sm:py-3 rounded-lg mb-4 sm:mb-6 shadow-md text-xs sm:text-sm">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                @if(session('error'))
                    <div
                        class="bg-red-100 border border-red-400 text-red-700 px-3 py-2 sm:px-4 sm:py-3 rounded-lg mb-4 sm:mb-6 shadow-md text-xs sm:text-sm">
                        {{ session('error') }}
                    </div>
                @endif
                ...
                @if($videos->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                        @foreach($videos as $video)
                            <div
                                class="bg-white rounded-lg sm:rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition duration-200">
                                <!-- Video preview -->
                                ...
                                <div class="p-3 sm:p-4">
                                    <h3 class="font-bold text-gray-900 text-sm sm:text-base mb-1 line-clamp-1">{{ $video->title }}
                                    </h3>
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <form action="{{ route('prestataire.videos.destroy', $video) }}" method="POST"
                ...
                @else
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-8 sm:p-12 text-center border border-gray-200">
                        <div
                            class="mx-auto w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 rounded-full flex items-center justify-center mb-4 sm:mb-6">
