                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    @foreach($videos as $video)
                        <div class="bg-white rounded-lg sm:rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition duration-200">
                            <!-- Video preview -->
                            <div class="aspect-video bg-gray-900 relative group cursor-pointer" onclick="playVideo(this)">
                                @if($video->status === 'processing')
                                    <div class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
                                        <div class="text-white text-center">
                                            <i class="fas fa-spinner fa-spin text-2xl mb-2"></i>
                                            <p class="text-xs">Traitement...</p>
                                        </div>
                                    </div>
                                @elseif($video->status === 'failed')
                                    <div class="absolute inset-0 flex items-center justify-center bg-red-100 bg-opacity-90 z-10">
                                        <div class="text-red-600 text-center p-2">
                                            <i class="fas fa-exclamation-circle text-2xl mb-2"></i>
                                            <p class="text-xs font-bold">Échec du traitement</p>
                                        </div>
                                    </div>
                                @endif

                                @if($video->video_path)
                                    <video class="w-full h-full object-cover video-player"
                                           preload="metadata"
                                           playsinline
                                           webkit-playsinline
                                    ...
                                    <!-- Overlay de lecture -->
                                    <div class="play-overlay absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 group-hover:bg-opacity-40 transition-all">
                                        <div class="w-16 h-16 rounded-full bg-white bg-opacity-90 flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform">
...
function playVideo(container) {
    ...
    document.querySelectorAll('.video-player').forEach(v => {
        if (v !== video && !v.paused) {
            ...
            const parentOverlay = v.parentElement.querySelector('.play-overlay');
    ...
    if (video.paused) {
        video.play().then(() => {
            ...
            if (src) {
                // Ouvrir directement la vidéo pour diagnostiquer (404 / permissions / type)
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.video-player').forEach(video => {
        ...
        video.addEventListener('pause', function() {
            // Ne pas cacher les contrôles si la vidéo n'est pas terminée
            if (this.currentTime < this.duration) {
                // Garder les contrôles visibles
            }
        });
    });
});
