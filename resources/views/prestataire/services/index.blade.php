...
<div class="min-h-screen bg-linear-to-br from-blue-50 to-indigo-100">
    ...
    <!-- En-tête moderne -->
    <div class="bg-white shadow-lg border-b-4 border-blue-600 mt-3">
    ...
    <div class="container mx-auto px-4 py-3 sm:py-4 md:py-8">
                    ...
                    <div class="bg-white rounded-lg sm:rounded-xl shadow-lg border border-blue-200 hover:shadow-xl transition duration-200 flex flex-col h-full">
                        ...
                        <div class="relative">
                            @if($service->images->isNotEmpty())
                                <x-media-image :path="$service->images->first()->image_path" :alt="$service->title" class="w-full h-32 sm:h-40 md:h-48 object-cover rounded-t-lg sm:rounded-t-xl" />
                            @else
        ...
        @else
            <x-empty-state 
                ...
                message="Vous n'avez pas encore créé de service. Les services sont le cœur de votre activité sur TaPrestation - ils permettent aux clients de découvrir ce que vous proposez et de réserver directement."
                :primaryAction="['url' => route('prestataire.services.create'), 'label' => 'Créer mon premier service']"
                :secondaryAction="['url' => route('prestataire.help.index'), 'label' => 'Comment ça marche ?']"
                :tips="[
                    'Ajoutez des photos de qualité de vos réalisations',
                    'Décrivez précisément vos prestations et tarifs',
                    'Indiquez votre zone d\'intervention',
                    'Activez la réservation en ligne pour plus de bookings'
                ]"
            />
        @endif
    </div>
        ...
        <div class="min-h-full w-full flex items-end sm:items-center justify-center p-3">
            <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl p-4 sm:p-6 w-full sm:max-w-md border border-red-200 transform transition-all duration-200 scale-95 opacity-0 modal-show">
                <div class="text-center">
                    ...
                    <p id="serviceTitle" class="text-base font-semibold text-blue-800 mt-2"></p>
