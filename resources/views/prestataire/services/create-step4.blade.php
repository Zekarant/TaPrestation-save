    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="max-w-4xl mx-auto">
            ...
            <form method="POST" action="{{ route('prestataire.services.store') }}" id="step4Form">
                ...
                <div class="bg-white rounded-xl shadow-lg border border-green-200 p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6">
                    ...
                    <div class="space-y-3 sm:space-y-4">
                        ...
                        <div class="flex flex-col space-y-2 pt-3 sm:pt-4 border-t border-green-200">
                            <span class="text-xs sm:text-sm font-medium text-gray-600">Matériel requis :</span>
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start space-y-1 sm:space-y-0 pt-3 sm:pt-4 border-t border-green-200">
                            <span class="text-xs sm:text-sm font-medium text-gray-600">Mode de paiement :</span>
                            <p class="text-blue-900 font-semibold text-sm sm:text-base sm:text-right">
                                @php
                                    $paymentReq = function_exists('normalize_payment_requirement_for_mode')
                                        ? normalize_payment_requirement_for_mode(session('service_creation.step2.payment_requirement', 'none'))
                                        : session('service_creation.step2.payment_requirement', 'none');
                                    $depositPct = session('service_creation.step2.deposit_percentage', 0);
                                    $cashOnlyMode = function_exists('cash_only_mode') && cash_only_mode();
                                @endphp
                                @if($cashOnlyMode)
                                    <span class="text-green-600"><i class="fas fa-money-bill-wave mr-1"></i>Paiement direct / en espèces</span>
                                @elseif($paymentReq === 'none')
                                    <span class="text-gray-600"><i class="fas fa-handshake mr-1"></i>Aucun paiement requis</span>
                                @elseif($paymentReq === 'deposit')
                                    <span class="text-amber-600"><i class="fas fa-percent mr-1"></i>Acompte de {{ $depositPct }}%</span>
                                @else
                                    <span class="text-green-600"><i class="fas fa-lock mr-1"></i>Paiement intégral</span>
                                @endif
                            </p>
                        </div>
                    </div>
                ...
                @include('components.payment-consent', ['type' => 'service', 'color' => 'blue'])
