        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-green-200 p-4 sm:p-6">
                ...
                <form action="{{ route('prestataire.equipment.store') }}" method="POST">
                    ...
                    <div class="space-y-4 sm:space-y-6 lg:space-y-8">
                        ...
                        <div>
                            ...
                            <div class="mt-4 sm:mt-6 pt-4 border-t border-gray-200">
                                <h4 class="font-semibold text-gray-900 mb-2 text-sm sm:text-base">Mode de paiement</h4>
                                @php
                                    $paymentReq = function_exists('normalize_payment_requirement_for_mode')
                                        ? normalize_payment_requirement_for_mode(session('equipment_creation.step2.payment_requirement', 'none'))
                                        : session('equipment_creation.step2.payment_requirement', 'none');
                                    $depositPct = session('equipment_creation.step2.deposit_percentage', 0);
                                    $securityDeposit = session('equipment_creation.step2.security_deposit', 0);
                                    $cashOnlyMode = function_exists('cash_only_mode') && cash_only_mode();
                                @endphp
                                <div class="flex flex-wrap gap-2">
                                    @if($cashOnlyMode)
                                        <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded"><i class="fas fa-money-bill-wave mr-1"></i>Paiement direct</span>
                                    @elseif($paymentReq === 'none')
                                        <span class="bg-gray-100 text-gray-800 text-xs px-2 py-1 rounded"><i class="fas fa-handshake mr-1"></i>Aucun paiement requis</span>
                                    @elseif($paymentReq === 'deposit')
                                        <span class="bg-amber-100 text-amber-800 text-xs px-2 py-1 rounded"><i class="fas fa-percent mr-1"></i>Acompte {{ $depositPct }}%</span>
                                    @else
                                        <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded"><i class="fas fa-lock mr-1"></i>Paiement intégral</span>
                                    @endif
                                    @if($securityDeposit > 0)
                                        <span class="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded"><i class="fas fa-shield-alt mr-1"></i>Caution {{ number_format($securityDeposit, 0, ',', ' ') }}€</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Consentement aux conditions de paiement -->
                    @include('components.payment-consent', ['type' => 'equipment', 'color' => 'green'])
