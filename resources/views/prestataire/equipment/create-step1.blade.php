                        ...
                        <div>
                            ...
                            <select id="category_id" name="category_id" required class="w-full px-3 py-2 text-sm sm:text-base border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 @error('category_id') border-red-500 @enderror">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ (string)old('category_id', session('equipment_step1.category_id')) === (string)$category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (categorySelect) {
        ...
        // Si une catégorie est déjà sélectionnée, charger les sous-catégories
