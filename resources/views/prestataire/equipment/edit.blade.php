...
<div class="bg-green-50">
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto">
            ...
            <form action="{{ route('prestataire.equipment.update', $equipment) }}" method="POST" enctype="multipart/form-data" id="equipment-form" x-data="{ pricingType: '{{ old('price_per_hour', $equipment->price_per_hour) ? 'hourly' : 'daily' }}' }">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-lg border border-green-200 p-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select x-model="pricingType" class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                                <option value="daily">Par jour (Standard)</option>
                                <option value="hourly">Par heure</option>
                            </select>
                        </div>
                        <div>
                            <label for="security_deposit" class="block text-sm font-medium text-green-700 mb-2">
                                Caution (€) <span class="text-green-500">*</span>
                            </label>
                            <input type="number" name="security_deposit" id="security_deposit" value="{{ old('security_deposit', $equipment->security_deposit) }}" required 
                                   min="0" step="0.01" placeholder="100" 
                                   class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('security_deposit') border-red-500 @enderror">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div x-show="pricingType === 'daily' || pricingType === 'hourly'" class="transition-all duration-300">
                            <label for="price_per_day" class="block text-sm font-medium text-green-700 mb-2">
                                Prix par jour (€) <span x-show="pricingType === 'daily'" class="text-green-500">*</span>
                            </label>
                            <input type="number" name="price_per_day" id="price_per_day" value="{{ old('price_per_day', $equipment->price_per_day) }}" :required="pricingType === 'daily'" 
                                   min="0" step="0.01" placeholder="50" 
                                   class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('price_per_day') border-red-500 @enderror">
                            @error('price_per_day')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div x-show="pricingType === 'hourly'" class="transition-all duration-300">
                            <label for="price_per_hour" class="block text-sm font-medium text-green-700 mb-2">Prix par heure (€) <span class="text-green-500">*</span></label>
                    ...
                    <div class="mt-6 pt-4 border-t border-green-200">
                        ...
                        <div id="earningsPreviewEquipment" class="p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200">
                    ...
                    <div class="mt-6 pt-4 border-t border-green-200">
                        ...
                        @if($cashOnlyMode)
                            ...
                            <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-sm text-green-800">
                                ...
                                <p class="mt-1">Cette location passe automatiquement en règlement direct avec le client. Les options acompte et paiement complet en ligne sont masquées.</p>
                            </div>
                        @else
                            <label for="payment_requirement" class="block text-sm font-medium text-green-700 mb-2">
                                <i class="fas fa-hand-holding-usd mr-1"></i>Paiement requis pour valider la location
                            </label>
                            <select id="payment_requirement" name="payment_requirement" 
                                    class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('payment_requirement') border-red-500 @enderror">
                                <option value="none" {{ old('payment_requirement', $equipment->payment_requirement ?? 'none') == 'none' ? 'selected' : '' }}>
                                    Aucun (réservation sans paiement immédiat)
                                </option>
                                <option value="deposit" {{ old('payment_requirement', $equipment->payment_requirement) == 'deposit' ? 'selected' : '' }}>
                                    Acompte obligatoire pour valider
                                </option>
                                <option value="full" {{ old('payment_requirement', $equipment->payment_requirement) == 'full' ? 'selected' : '' }}>
                ...
                <div class="bg-white rounded-xl shadow-lg border border-green-200 p-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div>
                            <label for="availability_start_time" class="block text-sm font-medium text-green-700 mb-2">
                                Heure de début de disponibilité
                            </label>
                            <input type="time" name="availability_start_time" id="availability_start_time" 
                                   class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('availability_start_time') border-red-500 @enderror"
                                   value="{{ old('availability_start_time', $equipment->availability_start_time ? \Carbon\Carbon::parse($equipment->availability_start_time)->format('H:i') : '') }}">
                            @error('availability_start_time')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="availability_end_time" class="block text-sm font-medium text-green-700 mb-2">
                                Heure de fin de disponibilité
                            </label>
                            <input type="time" name="availability_end_time" id="availability_end_time" 
                                   class="w-full px-3 py-2 border border-green-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('availability_end_time') border-red-500 @enderror"
                                   value="{{ old('availability_end_time', $equipment->availability_end_time ? \Carbon\Carbon::parse($equipment->availability_end_time)->format('H:i') : '') }}">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const currentSubcategoryId = @json((string) old('subcategory_id', $equipment->subcategory_id));
    const subcategoriesUrlTemplate = @json(route('categories.subcategories', ['category' => '__CATEGORY__']));
    ...
    if (categorySelect && subcategorySelect) {
        ...
        categorySelect.addEventListener('change', function() {
            ...
            if (categoryId) {
                const subcategoriesUrl = subcategoriesUrlTemplate.replace('__CATEGORY__', encodeURIComponent(categoryId));
                fetch(subcategoriesUrl, {
                    ...
                    .then(async (response) => {
                        ...
                        if (!contentType.includes('application/json')) {
                            throw new Error(`Réponse non JSON (${contentType || 'content-type manquant'})`);
                        }

                        try {
                            return JSON.parse(body);
                        } catch (e) {
                            throw new Error('JSON invalide reçu depuis le serveur');
                        }
                    })
                    .then(data => {
                        if (Array.isArray(data) && data.length > 0) {
                            data.forEach((subcategory) => {
                                ...
                                if (currentSubcategoryId && String(subcategory.id) === String(currentSubcategoryId)) {
