    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6 sm:mb-8">
            <div class="text-center mb-6 sm:mb-8 fade-in-up">
                ...
                <p class="text-lg sm:text-xl text-gray-700 px-4">Gérez vos informations personnelles et vos préférences</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            <div class="lg:col-span-3">
                <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 mb-6">
                        <div class="flex items-center gap-3 sm:gap-6 mb-4 sm:mb-6">
                            <div class="flex-shrink-0">
                                <div class="flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full">
                                    @if($prestataire && $prestataire->photo)
                                        <img class="h-full w-full rounded-full object-cover shadow-lg" src="{{ asset('storage/' . $prestataire->photo) }}" alt="Photo de profil">
                                    @else
                                        <span class="text-xl sm:text-2xl font-medium text-gray-700">{{ substr(auth()->user()->name, 0, 1) }}</span>
                                    @endif
                                </div>
                            </div>
                            <div class="flex-1">
                                ...
                                <div class="text-sm text-gray-700 mb-2 sm:mb-4">
                                    ...
                                    @if(auth()->user()->email_verified_at)
                                        <svg class="inline-block w-4 h-4 text-green-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                    @else
                                        <svg class="inline-block w-4 h-4 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                        </svg>
                                    @endif
                                </div>
                                <span class="text-sm @if(auth()->user()->email_verified_at) text-green-700 font-medium @else text-gray-500 @endif">Email vérifié</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-xl shadow-lg p-4 sm:p-6 mb-6">
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-purple-700 mb-3">Pour améliorer votre profil et attirer plus de clients, ajoutez au moins un service que vous proposez.</p>
                                <a href="{{ route('prestataire.services.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-sm font-bold rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 shadow-sm">
                                    Ajouter un service
                    ...
                    <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 mb-6">
                        ...
                        <form action="{{ route('prestataire.profile.update.personal') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mt-5 md:mt-0">
                                <div class="grid grid-cols-1 sm:grid-cols-6 gap-4 sm:gap-6 form-grid">
                                    ...
                                    <div class="col-span-1 sm:col-span-6 form-col-span">
                                        <label class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Photo de profil</label>
                                        <div class="mt-1 flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-5 mobile-stack">
                                            @if($prestataire && $prestataire->photo)
                                                <img class="h-20 w-20 rounded-full object-cover shadow-lg" src="{{ asset('storage/' . $prestataire->photo) }}" alt="Photo actuelle">
                                            @else
                                            ...
                                            <div class="flex flex-col space-y-2 w-full sm:w-auto mobile-full-width">
                                                <input type="file" name="photo" id="photo" accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-gray-50 file:text-gray-600 hover:file:bg-gray-100 mobile-text-sm">
                                                <p class="mt-1 text-xs text-gray-600 mobile-text-sm">Format recommandé : JPEG, PNG. Taille max : 2MB</p>
                                                @if($prestataire && $prestataire->photo)
                                                    <button type="button" onclick="deletePhoto()" class="delete-avatar-btn">Supprimer la photo</button>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        <label for="name" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Nom complet</label>
                                        <input type="text" name="name" id="name" value="{{ old('name', auth()->user()->name) }}" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm" required>
                                    </div>
                                    
                                    <!-- Email -->
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        <label for="email" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Email</label>
                                        <input type="email" name="email" id="email" value="{{ old('email', auth()->user()->email) }}" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm" required>
                                    </div>
                                    
                                    <!-- Téléphone -->
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        <label for="phone" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Téléphone</label>
                                        <input type="tel" name="phone" id="phone" value="{{ old('phone', $prestataire->phone ?? '') }}" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm">
                                    </div>
                                    
                                    <!-- Présentation / Biographie -->
                                    <div class="col-span-1 sm:col-span-6 form-col-span">
                                        <label for="description" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Présentation professionnelle</label>
                                        <textarea name="description" id="description" rows="4" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm" placeholder="Présentez votre expertise, votre expérience, vos points forts et votre manière de travailler...">{{ old('description', $prestataire->description ?? '') }}</textarea>
                                        <p class="mt-2 text-sm text-gray-600 mobile-text-sm mobile-mt-4">Décrivez votre expertise, votre expérience et ce qui vous différencie. Minimum 200 caractères.</p>
                                    </div>
                                </div>
                            </div>
                            
                            <!-- Bouton d'action pour les informations personnelles -->
                            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 mt-6 mobile-stack">
                                <button type="submit" class="inline-flex justify-center py-3 px-6 border border-transparent shadow-lg text-base font-bold rounded-lg text-white bg-gray-600 hover:bg-gray-700 transition duration-200 w-full sm:w-auto z-10 relative mobile-full-width">
                    ...
                    <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6">
                        <div class="section-header">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-700">Modifiez votre mot de passe pour renforcer la sécurité de votre compte.</p>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('prestataire.profile.update.security') }}" method="POST">
                            ...
                            <div class="mt-5 md:mt-0">
                                <div class="grid grid-cols-1 sm:grid-cols-6 gap-4 sm:gap-6 form-grid">
                                    <!-- Mot de passe actuel -->
                                    <div class="col-span-1 sm:col-span-6 form-col-span">
                                        <label for="current_password" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Mot de passe actuel</label>
                                        <input type="password" name="current_password" id="current_password" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm" required>
                                        <p class="mt-2 text-sm text-gray-600 mobile-text-sm mobile-mt-4">Entrez votre mot de passe actuel pour confirmer les modifications.</p>
                                    </div>
                                    
                                    <!-- Nouveau mot de passe -->
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        <label for="new_password" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Nouveau mot de passe</label>
                                        <div class="relative">
                                            <input type="password" name="new_password" id="new_password" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 pr-10 transition duration-200 mobile-p-3 mobile-text-sm" required>
                                    ...
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        <label for="new_password_confirmation" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Confirmer le nouveau mot de passe</label>
                                        <div class="relative">
                                            <input type="password" name="new_password_confirmation" id="new_password_confirmation" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 pr-10 transition duration-200 mobile-p-3 mobile-text-sm" required>
                            ...
                            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                <div class="flex flex-col sm:flex-row mobile-stack">
                                    ...
                                    <div class="sm:ml-4 flex-1">
                                        ...
                                        <div class="mt-4">
                                            <button type="button" onclick="openDeleteModal()" class="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-lg font-bold transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 w-full sm:w-auto z-10 relative mobile-full-width">
        ...
        <div class="mt-3 text-center">
            ...
            <div class="mt-4 px-7 py-3">
                ...
                <form id="deleteForm" method="POST" action="{{ route('prestataire.profile.destroy') }}">
                    ...
                    <div class="mb-4">
                        <label for="password" class="block text-md font-bold text-gray-800 mb-2">Saisissez votre mot de passe :</label>
                        <input type="password" id="password" name="password" required class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent transition duration-200 mobile-p-3 mobile-text-sm">
                    </div>
                    
                    <div class="mb-4">
                        <label for="confirmation" class="block text-md font-bold text-gray-800 mb-2">Tapez "DELETE" pour confirmer :</label>
                        <input type="text" id="confirmation" name="confirmation" required class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent transition duration-200 mobile-p-3 mobile-text-sm" placeholder="DELETE">
                    </div>
                    
                    <div class="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4 mt-6 mobile-stack mobile-space-y-3 mobile-space-x-0">
                        <button type="button" onclick="closeDeleteModal()" class="px-6 py-3 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition-colors font-bold mobile-full-width">
                            Annuler
                        </button>
                        <button type="submit" class="px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 mobile-full-width">
    ...
    <div class="bg-white rounded-lg sm:rounded-xl shadow-2xl p-4 sm:p-6 md:p-8 max-w-xs sm:max-w-md w-full mx-4 border-2 sm:border-4 border-red-500 transform transition-all duration-300 scale-95 opacity-0 modal-show">
        <div class="text-center">
            ...
            <div class="mt-4 sm:mt-6 flex flex-col gap-2 sm:gap-3">
                <button type="button" onclick="closeDeletePhotoModal()" class="flex-1 px-3 py-2 sm:px-4 sm:py-2.5 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition duration-200 font-medium text-sm sm:text-base">
                    Annuler
                </button>
                <button type="button" onclick="confirmDeletePhoto()" class="flex-1 px-3 py-2 sm:px-4 sm:py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition duration-200 font-medium text-sm sm:text-base">
                    Supprimer
                </button>
            </div>
        </div>
