...
<div class="min-h-screen bg-blue-50 py-8">
    <div class="max-w-8xl mx-auto px-3 sm:px-4 lg:px-6">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-blue-200 p-3 sm:p-4 mb-6">
            ...
            <form method="GET" action="{{ route('client.prestataire-follows.index') }}" class="space-y-4" id="filtersForm" style="display: none;">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select name="sort" id="sort" class="w-full pl-10 pr-4 py-2 text-sm rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" onchange="this.form.submit()">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Rechercher par nom..." class="w-full pl-10 pr-10 py-2 text-sm rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            @if(request('search'))
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200 shadow hover:shadow-md flex items-center justify-center text-sm">
                            Appliquer
                        </button>
                    </div>
                </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pt-3 border-t border-blue-200 mt-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="px-2 sm:px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs sm:text-sm font-bold">
                        {{ $prestataires->total() }} prestataire{{ $prestataires->total() > 1 ? 's' : '' }} suivi{{ $prestataires->total() > 1 ? 's' : '' }}
                    </span>
                </div>
            </div>
        </div>
        ...
        @if($prestataires->isEmpty())
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 sm:p-12 text-center">
                ...
                <p class="mt-2 text-gray-500 max-w-md mx-auto">Découvrez et suivez vos prestataires préférés pour rester informé de leurs dernières activités.</p>
                <div class="mt-6">
                    <a href="{{ route('prestataires.index') }}" class="inline-flex items-center px-5 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-linear-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 h-full flex flex-col">
                        <div class="p-6 flex-grow flex flex-col">
                            ...
                            <div class="flex items-start flex-shrink-0">
                                <div class="flex-shrink-0 relative">
                                    ...
                                    @elseif($prestataire->user->avatar)
                                        <img src="{{ asset('storage/' . $prestataire->user->avatar) }}" alt="{{ $prestataire->user->name ?? 'Prestataire' }}" 
                                            class="w-16 h-16 rounded-xl object-cover">
                                    @else
                                        <div class="w-16 h-16 rounded-xl bg-linear-to-r from-gray-200 to-gray-300 flex items-center justify-center">
                                ...
                                <div class="ml-4 flex-1 min-w-0">
                                    <div class="flex items-start justify-between">
                                        <div>
                                            <h3 class="text-lg font-semibold text-gray-900 truncate">{{ $prestataire->user->name ?? 'Prestataire' }}</h3>
                                        </div>
                                        @if($prestataire->isVerified())
                                    ...
                                    @if($prestataire->rating_average > 0)
                                        <div class="mt-1 flex items-center">
                                            ...
                                            <span class="ml-1 text-xs text-gray-500">{{ number_format($prestataire->rating_average, 1) }}/5</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="mt-2 grid grid-cols-2 gap-2">
                                        <div class="bg-blue-50 rounded-lg p-2 text-center">
                                            ...
                                            <p class="text-sm font-semibold text-blue-700">{{ $prestataire->services->count() }}</p>
                                        </div>
                                        <div class="bg-green-50 rounded-lg p-2 text-center">
                                            <p class="text-xs text-gray-500">Équipement à louer</p>
                                            <p class="text-sm font-semibold text-green-700">{{ $prestataire->equipments->count() }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 flex flex-col sm:flex-row sm:space-x-3 space-y-3 sm:space-y-0 flex-shrink-0">
                                <a href="{{ route('prestataires.show', $prestataire->id) }}" class="w-full sm:w-1/2 inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-linear-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700">
                                    <i class="fas fa-user mr-2"></i>
                                    <span class="sm:hidden">Profil</span>
                                    <span class="hidden sm:inline">Voir le profil</span>
                                </a>
                                <form action="{{ route('client.prestataire-follows.unfollow', $prestataire->id) }}" method="POST" class="w-full sm:w-1/2">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 border border-red-300 shadow-sm text-sm font-medium rounded-md text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 unfollow-button">
...
<div id="unfollowModal" class="fixed inset-0 z-50 hidden opacity-0 transition-opacity duration-200" style="backdrop-filter: blur(3px); background-color: rgba(0, 0, 0, 0.45);">
    <div class="min-h-full w-full flex items-end sm:items-center justify-center p-3">
        <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl p-4 sm:p-6 w-full sm:max-w-md border border-red-200 transform transition-all duration-200 scale-95 opacity-0 modal-show">
            <div class="text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mt-3">Confirmation de désabonnement</h3>
                <p class="text-gray-600 mt-2">
                    Êtes-vous sûr de vouloir vous désabonner de ce prestataire ?
                </p>
                <div class="mt-6 flex flex-col sm:flex-row gap-3">
                    <button id="cancelUnfollowBtn" class="flex-1 px-4 py-2 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition duration-200 font-medium">
                        Annuler
                    </button>
