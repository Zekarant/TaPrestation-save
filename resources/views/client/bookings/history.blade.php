...
@section('title', 'Historique de mes réservations')
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">📅 Historique de mes réservations</h1>
                <p class="text-gray-600 mt-1">Suivi complet de toutes vos prestations</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <form method="GET" action="{{ route('client.bookings.history') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                ...
                <p class="text-2xl font-bold text-green-600">{{ number_format($stats['total_spent'] ?? 0, 2) }} €</p>
            </div>
            <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
                <p class="text-sm text-gray-500">Moyenne par réservation</p>
        ...
        <div class="space-y-4">
                    ...
                    <div class="md:flex">
                        <div class="md:w-1/4 bg-gray-50 p-6 flex items-center justify-center">
                            @if($booking->service && $booking->service->image)
                                <x-media-image :path="$booking->service->image" :alt="$booking->service->name" class="w-full h-40 object-cover rounded" />
                            @else
                        ...
                        <div class="p-6 md:w-3/4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <p class="font-medium text-gray-900">{{ $booking->start_datetime->diffInHours($booking->end_datetime) }}h</p>
                                </div>
                                <div>
                            ...
                            <div class="flex gap-2 justify-start pt-4 border-t border-gray-100">
                                <a href="{{ route('client.bookings.show', $booking) }}" class="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-sm font-medium transition">
                                    Voir détails
                                </a>
                                @if($booking->status === 'completed')
                                    <a href="{{ route('reviews.create', ['booking_id' => $booking->id]) }}" class="px-4 py-2 bg-purple-50 hover:bg-purple-100 text-purple-600 rounded-lg text-sm font-medium transition">
                                        Laisser un avis
                                    </a>
                                @endif
                                @if($booking->status === 'confirmed')
                                    <button class="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-sm font-medium transition" onclick="confirmCancel({{ $booking->id }})">
                                        Annuler
                                    </button>
                                @endif
                            </div>
            ...
            @empty
                <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-8 text-center">
                    ...
                    <a href="{{ route('services.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
