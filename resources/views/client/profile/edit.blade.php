            ...
            <div class="lg:col-span-3">
                <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 mb-6">
                        <div class="section-header">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-700">Ces informations seront visibles par les prestataires.</p>
                            </div>
                        </div>
                        <form action="{{ route('client.profile.update.personal') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-6 gap-4 sm:gap-6 form-grid">
                                    ...
                                    <div class="col-span-1 sm:col-span-6 form-col-span">
                                        ...
                                        <div class="mt-1 flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-5 mobile-stack">
                                            @if(auth()->user()->client && auth()->user()->client->photo)
                                                <img class="h-20 w-20 rounded-full object-cover shadow-lg" src="{{ asset('storage/' . auth()->user()->client->photo) }}" alt="Photo actuelle">
                                            @else
                                    ...
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        ...
                                        <input type="tel" name="phone" id="phone" value="{{ old('phone', auth()->user()->client->phone ?? '') }}" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm">
                                    </div>
                                    
                                    <!-- Localisation -->
                                    <div class="col-span-1 sm:col-span-3 form-col-span">
                                        <label for="address" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Localisation</label>
                                        <input type="text" name="address" id="address" value="{{ old('address', auth()->user()->client->address ?? '') }}" placeholder="Ville, Région" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm">
                                    </div>
                                    
                                    <!-- Bio -->
                                    <div class="col-span-1 sm:col-span-6 form-col-span">
                                        <label for="bio" class="block text-sm font-semibold text-gray-700 mb-2 mobile-mb-2">Présentation</label>
                                        <textarea name="bio" id="bio" rows="4" class="mt-1 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-lg px-4 py-3 transition duration-200 mobile-p-3 mobile-text-sm" placeholder="Présentez-vous en quelques mots...">{{ old('bio', auth()->user()->client->bio ?? '') }}</textarea>
                                        <p class="mt-2 text-sm text-gray-600 mobile-text-sm mobile-mt-4">Décrivez vos besoins habituels, vos préférences ou votre secteur d'activité.</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="profile-card bg-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6">
                        ...
                        <form action="{{ route('client.profile.update.security') }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mt-5 md:mt-0">
                                <div class="grid grid-cols-1 sm:grid-cols-6 gap-4 sm:gap-6 form-grid">
...
function deletePhoto() {
    // Set the service title for the modal
...
function confirmDeletePhoto() {
    ...
    form.method = 'POST';
    form.action = '{{ route("client.profile.delete-avatar") }}';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (newPasswordInput) {
        newPasswordInput.addEventListener('input', function() {
            ...
            checkSubmitButton(allRequirementsMet, confirmPasswordInput ? confirmPasswordInput.value : '');
        });
    }
    ...
    if (confirmPasswordInput) {
        confirmPasswordInput.addEventListener('input', function() {
            ...
            const allRequirementsMet = updatePasswordRequirements(password);
            checkSubmitButton(allRequirementsMet, confirmPassword);
        });
    }
    ...
    function handleResponsiveBehavior() {
        ...
        const modalContent = deleteModal ? deleteModal.querySelector('.relative') : null;
        
        if (modalContent && screenWidth <= 640) {
            // Mobile view
            ...
            // Desktop view
            modalContent.style.width = '24rem'; // w-96
            modalContent.style.margin = '0 auto';
            modalContent.style.top = '5rem'; // top-20
        }
    }
...
function checkSubmitButton(allRequirementsMet, confirmPassword) {
    const submitButton = document.getElementById('submit-btn');
    const newPassword = document.getElementById('new_password') ? document.getElementById('new_password').value : '';
    
    // If no password is entered, keep button disabled but in default state
    if (!newPassword) {
