                        ...
                        <div class="flex-shrink-0">
                            @if($rental->equipment->photos && count($rental->equipment->photos) > 0)
                                @php
                                    $firstPhoto = null;
                                    try {
                                        if(isset($rental->equipment->photos[0]) && \Illuminate\Support\Facades\Storage::disk('public')->exists($rental->equipment->photos[0])) {
                                            $firstPhoto = \Illuminate\Support\Facades\Storage::url($rental->equipment->photos[0]);
                                        }
                                    } catch (\Exception $e) {
                                        $firstPhoto = null;
                                    }
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">{{ number_format($rental->equipment->security_deposit, 0) }}€</span>
                        </div>
                        @endif
                        ...
                        @php
                            $depositAmount = (float) ($rental->equipment->security_deposit ?? $rental->security_deposit ?? 0);
                            $depositStatus = strtolower((string) ($rental->rentalRequest->deposit_status ?? 'pending'));
                            $depositRetained = (float) ($rental->rentalRequest->deposit_retained ?? $rental->deposit_retained ?? 0);
                            $depositReturned = (float) ($rental->deposit_returned ?? max(0, $depositAmount - $depositRetained));
                        @endphp
                        @if($depositAmount > 0)
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600">Statut caution:</span>
                                @if($depositStatus === 'returned')
                                    <span class="text-green-600 font-medium">Remboursée ({{ number_format($depositReturned, 0) }}€)</span>
                                @elseif($depositStatus === 'partial')
                                    <span class="text-orange-600 font-medium">Partielle (retenu {{ number_format($depositRetained, 0) }}€)</span>
                                @elseif($depositStatus === 'retained')
                                    <span class="text-red-600 font-medium">Retenue ({{ number_format($depositRetained, 0) }}€)</span>
                                @else
