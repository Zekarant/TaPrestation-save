...
@section('content')
<div class="min-h-screen bg-linear-to-br from-slate-900 via-blue-900 to-slate-900">
<div class="container mx-auto px-4 py-6 sm:py-8">
    <div class="max-w-6xl mx-auto">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
            <h1 class="text-2xl sm:text-3xl font-bold text-cyan-300">Mes Réservations</h1>
            @if(auth()->user()->role === 'client')
                <a href="{{ route('services.index') }}" class="bg-linear-to-r from-blue-500 to-cyan-500 hover:shadow-lg hover:shadow-blue-500/50 text-cyan-50 px-4 py-2 sm:px-6 sm:py-2.5 rounded-lg transition duration-200 text-sm sm:text-base font-bold">
        ...
        @if($bookings->count() > 0)
            <div class="grid gap-4 sm:gap-6">
                @foreach($bookings as $booking)
                    <div class="bg-slate-800 rounded-lg shadow-lg border border-blue-700/30 p-4 sm:p-6">
                        <div class="flex flex-col lg:flex-row justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                                    <h3 class="text-lg sm:text-xl font-semibold text-blue-200">
                                ...
                                <div class="text-cyan-300 mb-2 text-sm sm:text-base">
                                    <strong>Numéro de réservation:</strong> {{ $booking->booking_number }}
                                </div>
                                
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm text-blue-200">
                                    <div>
                                        <strong>Date et heure:</strong><br>
                                        {{ $booking->start_datetime ? $booking->start_datetime->format('d/m/Y à H:i') : 'Non définie' }}
                                ...
                                @if($booking->client_notes)
                                    <div class="mt-3 sm:mt-4 p-3 bg-blue-50 rounded border border-blue-200">
                                        <strong class="text-xs sm:text-sm text-cyan-300">Notes du client:</strong>
                                        <p class="text-xs sm:text-sm text-blue-200 mt-1">{{ $booking->client_notes }}</p>
                                    </div>
                                @endif
                                
                                @if($booking->prestataire_notes)
                                    <div class="mt-3 sm:mt-4 p-3 bg-blue-900/30 rounded border border-blue-700/30">
                                        <strong class="text-xs sm:text-sm text-cyan-300">Notes du prestataire:</strong>
                                        <p class="text-xs sm:text-sm text-blue-200 mt-1">{{ $booking->prestataire_notes }}</p>
                                    </div>
                                @endif
                                
                                @if($booking->status === 'cancelled' && $booking->cancellation_reason)
                                    <div class="mt-3 sm:mt-4 p-3 bg-red-900/30 rounded border border-red-700/30">
                                        <strong class="text-xs sm:text-sm text-red-300">Raison de l'annulation:</strong>
                                        <p class="text-xs sm:text-sm text-red-200 mt-1">{{ $booking->cancellation_reason }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex flex-row lg:flex-col gap-2 lg:gap-3 lg:ml-4 w-full lg:w-auto">
                                <a href="{{ route('bookings.show', $booking) }}" 
                                   class="bg-blue-900/50 hover:bg-blue-800/50 text-cyan-300 px-3 py-2 sm:px-4 sm:py-2.5 rounded text-xs sm:text-sm transition duration-200 text-center border border-blue-700/50 flex-1 lg:flex-none">
                                    Voir détails
                                </a>
                                ...
                                @if(auth()->user()->role === 'prestataire')
                                    @if($booking->status === 'pending')
                                        <form action="{{ route('bookings.confirm', $booking) }}" method="POST" class="inline flex-1 lg:flex-none">
                                            ...
                                            @method('PATCH')
                                            <button type="submit" 
                                                    class="bg-linear-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-cyan-50 px-3 py-2 sm:px-4 sm:py-2.5 rounded text-xs sm:text-sm transition duration-200 w-full font-bold">
                                                Confirmer
                                            </button>
                                        </form>
                                    @elseif($booking->status === 'confirmed')
                                        <form action="{{ route('bookings.complete', $booking) }}" method="POST" class="inline flex-1 lg:flex-none">
                                            ...
                                            @method('PATCH')
                                            <button type="submit" 
                                                    class="bg-linear-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-cyan-50 px-3 py-2 sm:px-4 sm:py-2.5 rounded text-xs sm:text-sm transition duration-200 w-full font-bold">
                                                Marquer terminé
                                            </button>
                                        </form>
                                    @endif
                        ...
                        <button type="button" data-booking-id="{{ $booking->id }}" class="cancel-btn bg-linear-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-red-50 px-3 py-2 sm:px-4 sm:py-2.5 rounded text-xs sm:text-sm transition duration-200 flex-1 lg:flex-none font-bold">
                                        Annuler
                                    </button>
                                @endif
                            </div>
