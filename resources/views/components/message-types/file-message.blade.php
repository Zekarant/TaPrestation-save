...
<div class="message-bubble">
    ...
    @elseif($isVideo)
        ...
        @php
            $videoMimeTypes = ['mp4' => 'video/mp4', 'webm' => 'video/webm', 'ogg' => 'video/ogg', 'avi' => 'video/x-msvideo', 'mov' => 'video/quicktime', 'm4v' => 'video/mp4'];
            $videoMimeType = $videoMimeTypes[strtolower($fileExtension)] ?? 'video/mp4';
        @endphp
        <div class="video-preview mb-2">
            <video controls playsinline webkit-playsinline x-webkit-airplay="allow" class="max-w-xs max-h-64 rounded-lg">
                <source src="/storage/{{ $message->file_path }}" type="{{ $videoMimeType }}">
...
function previewFile(filePath, fileName, fileExtension) {
    ...
    if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'].includes(fileExtension.toLowerCase())) {
        ...
        const mimeTypes = { mp4: 'video/mp4', webm: 'video/webm', ogg: 'video/ogg', avi: 'video/x-msvideo', mov: 'video/quicktime' };
        const mimeType = mimeTypes[fileExtension.toLowerCase()] || 'video/mp4';
        content.innerHTML = `
            <video controls playsinline webkit-playsinline x-webkit-airplay="allow" class="max-w-full h-auto mx-auto">
                <source src="/storage/${filePath}" type="${mimeType}">
                Votre navigateur ne supporte pas la lecture vidéo.
            </video>
        `;
