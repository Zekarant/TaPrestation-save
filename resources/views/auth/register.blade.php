...
/* Removed subcategory-group CSS as per requirements */
/* #subcategory-group {
    transition: all 0.3s ease;
} */
                ...
                <div id="prestataire-form" class="form-section">
                    <form id="prestataire-form-element" class="mt-8 space-y-6" action="{{ route('register') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="profile-picture-container">
                            ...
                            <input type="file" id="prestataire_profile_photo" name="prestataire_profile_photo" class="form-control-file" accept="image/*" required style="display: none;">
                        </div>
...
// Add category and subcategory variables
let categories = [];
let subcategories = [];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle client profile picture click
    ...
    // Handle prestataire profile picture click
    ...
    // Load categories when prestataire form is selected
    ...
    if (document.getElementById('category_id')) {
            ...
            if (categoryId) {
                ...
                fetch(`/api/categories/${categoryId}/subcategories`)
                    ...
                    .then(data => {
                        ...
                        // If there was a previously selected subcategory (from validation errors), select it
        ...
        // Trigger change event on page load if a category is already selected (for validation errors)
        const categorySelect = document.getElementById('category_id');
        if (categorySelect && categorySelect.value) {
            // Dispatch the change event after a short delay to ensure DOM is ready
            setTimeout(() => {
                ...
                if (input.name !== '_token') {
                    ...
                    if (form.id === 'client-form-element') {
                        ...
                        const requiredFields = [
                            ...
                            'prestataire_profile_photo', 'category_id', 'subcategory_id'
                        ];
                        ...
                        if (input.id === 'prestataire_location') {
                            input.setAttribute('required', 'required');
                        }
                    }
                }
    ...
    userTypeOptions.forEach(option => {
        option.addEventListener('click', function() {
            ...
            if (userType === 'client') {
                ...
                // reassuranceText element doesn't exist, so we'll remove this line
                ...
                setTimeout(() => {
                    ...
                    // Load categories when prestataire form is activated
                    console.log('Loading categories for prestataire form');
                    loadMainCategories();
                }, 10);
                // reassuranceText element doesn't exist, so we'll remove this line
            }
    ...
    // If prestataire form is already active (e.g., after validation error), load categories
    if (prestataireForm.classList.contains('active')) {
        loadMainCategories();
    }
    ...
    // Add autocomplete functionality for address fields
    ...
    // Create autocomplete dropdown elements
...
const clientPasswordConfirmation = document.getElementById('client_password_confirmation');
