            ...
            <form method="POST" action="{{ route('administrateur.notifications.send') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group" id="specific_users_group" style="display: none;">
                        ...
                        {{-- Audit 3.10: champ texte avec recherche AJAX au lieu de charger TOUS les users --}}
                        <input type="text" id="user_search" class="form-control mb-2" placeholder="Rechercher un utilisateur (nom ou email)..." autocomplete="off">
                        <select name="user_ids[]" id="user_ids" class="form-control" multiple size="8">
                        </select>
                        <small class="form-text text-muted">Tapez au moins 2 caractères pour rechercher. Maintenez Ctrl/Cmd pour sélectionner plusieurs.</small>
                    </div>
...
$(document).ready(function() {
    ...
    // Audit 3.10: Recherche AJAX users au lieu de tout charger
    let searchTimeout = null;
    $('#user_search').on('input', function() {
        clearTimeout(searchTimeout);
        const q = $(this).val().trim();
        if (q.length < 2) { $('#user_ids').empty(); return; }
        searchTimeout = setTimeout(function() {
            $.getJSON("{{ route('admin.users.search') }}", { q: q }, function(data) {
                const select = $('#user_ids');
                const selected = select.val() || [];
                select.find('option').each(function() {
                    if (selected.indexOf($(this).val()) === -1) $(this).remove();
                });
                data.forEach(function(u) {
                    if (!select.find('option[value="' + u.id + '"]').length) {
                        select.append('<option value="' + u.id + '">' + $('<span>').text(u.name + ' (' + u.email + ')').html() + '</option>');
                    }
                });
            });
        }, 300);
