...
<div class="bg-blue-50">
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8">
        <div id="filtersPanel" class="bg-white rounded-xl shadow-lg border border-blue-200 p-4 sm:p-6 mb-6 sm:mb-8" style="display: none;">
            ...
            <form action="{{ route('admin.messages.index') }}" method="GET" class="space-y-4 sm:space-y-6">
                ...
                <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 sm:pt-6 border-t-2 border-blue-200">
                    ...
                    @if(request('type') || request('status') || request('read_status') || request('reported') || request('date_from') || request('date_to') || request('sender') || request('recipient') || request('search'))
                        <a href="{{ route('admin.messages.index') }}" class="bg-white hover:bg-gray-50 text-blue-600 border border-blue-200 font-bold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg transition duration-200 flex items-center justify-center text-sm sm:text-base">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-blue-100 overflow-hidden">
        <!-- En-tête -->
        <div class="bg-linear-to-r from-blue-600 to-blue-700 px-4 sm:px-6 py-4 sm:py-5">
        ...
        <div class="p-4 sm:p-6">
                            ...
                            @foreach($messages as $message)
                                <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 {{ !$message->is_read ? 'border-l-4 border-l-yellow-400 bg-yellow-50' : '' }} {{ $message->is_reported ? 'border-l-4 border-l-red-400 bg-red-50' : '' }}">
                                    <div class="p-4 sm:p-6">
                                        ...
                                        <!-- Actions -->
                                        <div class="flex flex-wrap gap-2">
                                            <a href="{{ route('admin.messages.show', $message->id) }}" class="inline-flex items-center px-3 py-1.5 border border-blue-300 text-xs font-medium rounded-md text-blue-700 bg-blue-50 hover:bg-blue-100 transition-colors duration-200">
                                                <i class="fas fa-eye mr-1"></i> Voir
                                            </a>
                                            
                                            @if($message->moderation_status !== 'approved')
                                                <button type="button" class="inline-flex items-center px-3 py-1.5 border border-green-300 text-xs font-medium rounded-md text-green-700 bg-green-50 hover:bg-green-100 transition-colors duration-200 opacity-50 cursor-not-allowed" disabled title="Fonction en développement">
                                                    <i class="fas fa-check mr-1"></i> Approuver
                                                </button>
                                            @endif
                                            
                                            @if($message->moderation_status !== 'hidden')
                                                <button type="button" class="inline-flex items-center px-3 py-1.5 border border-yellow-300 text-xs font-medium rounded-md text-yellow-700 bg-yellow-50 hover:bg-yellow-100 transition-colors duration-200 opacity-50 cursor-not-allowed" disabled title="Fonction en développement">
            ...
            @else
                ...
                <div class="text-center py-12 sm:py-16">
                    ...
                    <a href="{{ route('admin.messages.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition duration-200">
...
function bulkModerate(action) {
    ...
    if (confirm(`${actionText[action]} ${selected.length} message(s) ?`)) {
        alert('Fonction de modération en masse en développement');
    }
}
...
function bulkMarkAsRead() {
    ...
    if (confirm(`Marquer ${selected.length} message(s) comme lu(s) ?`)) {
        alert('Fonction de marquage en masse en développement');
    }
}
...
function bulkDelete() {
    ...
    if (confirm(`Supprimer définitivement ${selected.length} message(s) ?`)) {
        alert('Fonction de suppression en masse en développement');
    }
}
