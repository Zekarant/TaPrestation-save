            ...
            <header class="messaging-header bg-white border-b border-blue-200 sticky top-0 z-20 w-full">
                <div class="conversation-header p-4 md:p-6">
                        ...
                        <div class="flex items-center flex-1 min-w-0">
                            <div class="conversation-avatar flex-shrink-0 relative mr-3 sm:mr-4">
                                @if(isset($otherUser))
                                    ...
                                    @else
                                        <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-bold text-base sm:text-lg border-2 border-blue-200">
                                            {{ strtoupper(substr($otherUser->name, 0, 1)) }}
                                        </div>
                                    @endif
                                    <div class="absolute -bottom-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 rounded-full border-2 border-white status-dot-avatar {{ ($otherUser->is_online ?? false) ? 'bg-green-500' : 'bg-gray-400' }}"></div>
                                @else
                                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-200 flex items-center justify-center text-blue-800 font-bold text-base sm:text-lg border-2 border-blue-300">
                                        ?
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="conversation-info flex-1 min-w-0">
                                <h2 class="text-lg sm:text-xl font-bold text-blue-900 truncate">{{ $otherUser->name ?? 'User' }}</h2>
                                <div class="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-blue-600 mt-1">
                                    @if(isset($otherUser) && $otherUser->role === 'prestataire')
                                        <span class="flex items-center px-1.5 py-0.5 sm:px-2 sm:py-0.5 bg-blue-100 text-blue-700 rounded-full font-medium">
                                            <i class="fas fa-tools mr-1 text-xs"></i>
                                            <span class="text-xs sm:text-sm">Prestataire</span>
                                        </span>
                                    @elseif(isset($otherUser))
                                        <span class="flex items-center px-1.5 py-0.5 sm:px-2 sm:py-0.5 bg-green-100 text-green-700 rounded-full font-medium">
                                    ...
                                    @if(isset($otherUser) && ($otherUser->is_online ?? false))
                                        <span class="flex items-center text-green-600 online-status-indicator">
                                            <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-500 rounded-full mr-1 sm:mr-2 animate-pulse status-dot"></span>
                                            <span class="text-xs sm:text-sm">En ligne</span>
                                        </span>
                                    @else
                                        <span class="flex items-center text-gray-400 online-status-indicator">
                                            <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-gray-400 rounded-full mr-1 sm:mr-2 status-dot"></span>
                                            <span class="text-xs sm:text-sm">{{ isset($otherUser) ? ($otherUser->online_status ?? 'Hors ligne') : 'Hors ligne' }}</span>
                                        </span>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="flex items-center space-x-2 sm:space-x-3">
                            <a href="{{ Auth::user()->hasRole('client') ? route('messaging.index') : (Auth::user()->hasRole('prestataire') ? route('prestataire.prestataire.messages.index') : '#') }}" class="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 hover:text-blue-900 transition-all duration-200">
                                <i class="fas fa-arrow-left text-sm sm:text-base"></i>
                            </a>
                            
                            <div class="relative" x-data="{ open: false }">
                                <button @click="open = !open" class="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 hover:text-blue-900 transition-all duration-200" title="Plus d'options">
                    ...
                    <div class="px-4 md:px-6 py-4 flex-1" id="messages-list">
                        @forelse($messages as $message)
                            <div class="mb-4 {{ $message->sender_id === Auth::id() ? 'flex justify-end' : 'flex justify-start' }}" 
                                ...
                                <div class="max-w-xs {{ $message->sender_id === Auth::id() ? 'bg-blue-600 text-white' : 'bg-white border border-blue-200' }} rounded-2xl px-3 py-2 sm:px-4 sm:py-2 shadow-sm">
...
<div class="message-input-container bg-gray-50 border-t-2 border-blue-200 px-4 md:px-6 py-2 w-full">
                    <div class="w-full max-w-7xl mx-auto">
                        <form id="message-form" action="{{ route('messaging.send.ajax') }}" method="POST" class="flex items-end space-x-2">
                            @csrf
                            ...
                            <div class="flex-1 relative">
                                ...
                                <div class="flex items-center justify-between mt-1 px-1">
                                    <div class="typing-indicator text-xs text-blue-600 hidden sm:block" id="typing-indicator" style="display: none;">
                            ...
                            <button type="submit" class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-blue-600 hover:bg-blue-700 text-white transition-colors duration-200 shadow-md hover:shadow-lg" id="send-button">
            ...
            if (messageInput && charCount) {
                ...
                messageInput.addEventListener('keypress', function(e) {
                    ...
                    if (e.key === 'Enter' && !e.shiftKey) {
                        e.preventDefault();
                        // Send message via AJAX instead of form submission
                        sendMessage();
                    }
                });
            }
