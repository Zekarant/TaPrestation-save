...
<div class="bg-blue-50 min-h-screen py-4 sm:py-6 md:py-8">
    <div class="max-w-4xl mx-auto px-3 sm:px-4 lg:px-8">
        <!-- PWA Back Button -->
        <div class="mb-4">
            <x-pwa-back-button :href="route('home')" />
        </div>
            ...
            <div class="bg-white rounded-xl shadow-lg border border-blue-200 overflow-hidden">
                <div class="divide-y divide-blue-100">
                    @foreach($notifications as $notification)
                        ...
                        @endphp
                        @php
                            // Préparation de l'URL d'action pour la notification
                            $notifActionUrl = null;
                            if (is_array($notification->data)) {
                                $notifActionUrl = $notification->data['url'] ?? $notification->data['action_url'] ?? null;
                            } elseif (is_string($notification->data)) {
                                $decodedData = json_decode($notification->data, true);
                                $notifActionUrl = $decodedData['url'] ?? $decodedData['action_url'] ?? null;
                            }
                            $redirectUrl = $notifActionUrl ? route('notifications.redirect', $notification->id) : null;
                        @endphp
                        <div class="p-4 sm:p-5 md:p-6 hover:{{ $cardBgClass }} transition-colors duration-200 {{ !$notification->read_at ? $cardBgClass . ' ' . $borderClass : '' }} {{ $redirectUrl ? 'cursor-pointer' : '' }}" @if($redirectUrl) onclick="window.location.href='{{ $redirectUrl }}'" @endif>
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                                <div class="flex items-start space-x-3 sm:space-x-4 flex-1 min-w-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if(!empty($actionUrl))
                                            <a href="{{ route('notifications.redirect', $notification->id) }}" class="text-sm {{ $linkClass }} font-medium transition-colors duration-200">
